...
const INITIALIZE_DISCRIMINATOR = Buffer.from([175, 238, 208, 100, 126, 101, 92, 182]);
const ADD_WHITELIST_DISCRIMINATOR = Buffer.from([126, 180, 243, 103, 26, 78, 112, 20]);
const ADD_BLACKLIST_DISCRIMINATOR = Buffer.from([218, 149, 124, 58, 186, 75, 96, 80]);
const EXECUTE_HOOK_DISCRIMINATOR = Buffer.from([140, 169, 145, 79, 117, 92, 168, 239]);
...
async function testAddWhitelist(configPda) {
  ...
  const [whitelistPda] = web3.PublicKey.findProgramAddressSync(
    [Buffer.from('whitelist'), wallet.publicKey.toBuffer(), testAddress.toBuffer()],
    new web3.PublicKey(PROGRAM_ID)
  );
  ...
  const ix = new web3.TransactionInstruction({
    keys: [
      { pubkey: new web3.PublicKey(configPda), isSigner: false, isWritable: false },
...
async function testAddBlacklist(configPda) {
  ...
  const badActor = new web3.PublicKey('ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL');
  const [blacklistPda] = web3.PublicKey.findProgramAddressSync(
    [Buffer.from('blacklist'), wallet.publicKey.toBuffer(), badActor.toBuffer()],
    new web3.PublicKey(PROGRAM_ID)
  );
...
async function main() {
  ...
  // Save results
  fs.writeFileSync('/root/sss2-transfer-hook/sss2_hook/app/test-results.json', JSON.stringify(results, null, 2));
