...
const PROGRAM_ID = new web3.PublicKey('FSkkSmrThcLpU9Uybrn4xcpbQKswUJn7KvoUQBsLPExD');
...
async function testFetchConfig(configPda) {
  ...
  try {
    ...
    console.log('  Permanent Delegate:', config.permanentDelegate?.toString() || 'None');
...
async function main() {
  ...
  if(results.togglePause?.pauseTx) console.log('Pause:', results.togglePause.pauseTx);
  if(results.togglePause?.unpauseTx) console.log('Unpause:', results.togglePause.unpauseTx);
