...
import { SolanaStablecoin, RoleManager, ROLE_MASTER, ROLE_MINTER, ROLE_BURNER, ROLE_PAUSER } from '../sdk/src';
...
async function main() {
  ...
  console.log('----------------------------------------');
  console.log(`Master Authority: ${masterAuthority.publicKey.toBase58().slice(0, 20)}...`);
  console.log(`\nCandidates:`);
  console.log(`  Minter:  ${minterCandidate.publicKey.toBase58().slice(0, 20)}...`);
  console.log(`  Burner:  ${burnerCandidate.publicKey.toBase58().slice(0, 20)}...`);
  console.log(`  Pauser:  ${pauserCandidate.publicKey.toBase58().slice(0, 20)}...`);
  ...
  const minterRole = await roleManager.getRolePDA(minterCandidate.publicKey, mint);
  const burnerRole = await roleManager.getRolePDA(burnerCandidate.publicKey, mint);
  const pauserRole = await roleManager.getRolePDA(pauserCandidate.publicKey, mint);
